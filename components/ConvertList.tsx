...
const ConvertList: React.FC<PropsWithChildren<ConvertListProps>> = ({ queue, handleRemoveFromQueue, changeConvertTo }) => {
  return (
    <div className='w-[80vw] bg-gray-800 rounded-md p-2 flex flex-col gap-2'>
      {queue.map((action, idx) => <ConvertListItem action={action} handleRemoveFromQueue={handleRemoveFromQueue} changeConvertTo={changeConvertTo} />)}
    </div>
  )
}
...
const ConvertListItem: React.FC<ConvertListItemProps> = ({ action, changeConvertTo, handleRemoveFromQueue }) => {
